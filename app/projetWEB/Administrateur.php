<html>
<menu>
<link href="css/Reseau.css" rel="stylesheet">
<div id="menu">
  <ul id="onglets">
    <li><a href="Accueil.php"> Accueil </a></li>
	<li><a href="Vous.php"> Vous </a></li>
    <li class="active"><a href="Reseau.php"> Reseau </a></li>
    <li><a href="Notif.php"> Notif </a></li>
    <li><a href="Emplois.php"> Emplois </a></li>
    <li><a href="AdministrateurTest.php"> Gestion des utilisateurs </a></li>
    <li><a href="Connexion.php"> Deconnexion </a></li>
  </ul>
</div>
</menu>
<h1>Administrateur - Gestion des utilisateurs</h1>
<input type="Button" class="boutons" value="Ajout d'un utilisateur"  Onclick="location.href='AdministrateurAjout.php'">
<input type="Button" class="boutons" value="Suppression d'un utilisateur" Onclick="location.href='AdministrateurSuppression.php'">
</html>
