<html>
<menu>
<link href="css/jobsupp.css" rel="stylesheet">
...
echo"pb de connection";}
	...
	echo '<div class="police">
			Emplois proposés: </br></br> 
		  </div>';
	while($data=mysqli_fetch_assoc($res)){
		echo '
		<div class="col-lg-4">
		<div class="marge">
		Emploi '.$data['ID_job'].': </br>'.'Nom du poste associé: '.$data['Nom_job'].'</br></br>
		</div>
		</div>
		';
		if(($data['ID_job']==$ID)&&($data['Nom_job']==$Nom)){
			if ($Nom!=""){
				...
				echo "<input type='button' class=boutons value='Confirmer la suppression.' Onclick=".$destination.">";
				}else{echo'Nom vide';}
			}
		}
	}
